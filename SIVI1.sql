...
(
   ID_CODIGO_PROVEEDOR	INT NOT NULL AUTO_INCREMENT,
   ID_PROVEEDOR         INT NOT NULL,
   ID_INSUMO            INT NOT NULL,
   CODIGO_INSUMO        VARCHAR(32) NOT NULL,
   PRIMARY KEY (ID_CODIGO_PROVEEDOR)
);
...
(
   ...
   ID_CODIGO_PROVEEDOR	INT NOT NULL,
...
(
   ...
   DESPACHO_LOCAL_GUIA  BOOL NOT NULL DEFAULT FALSE,
...
(
   ...
   COMPATIBILIDAD_INSUMO BOOL NOT NULL DEFAULT FALSE,
...
(
   ...
   DESPACHADA_VENTA     BOOL NOT NULL DEFAULT FALSE,
...
ALTER TABLE USUARIO ADD CONSTRAINT FK_RELATIONSHIP_48 FOREIGN KEY (ID_LOCAL)
...
ALTER TABLE DETALLE_COTIZACION ADD CONSTRAINT FK_RELATIONSHIP_66 FOREIGN KEY (ID_CODIGO_PROVEEDOR)
      REFERENCES CODIGO_PROVEEDOR (ID_CODIGO_PROVEEDOR) ON DELETE RESTRICT ON UPDATE CASCADE;
...
ALTER TABLE DETALLE_GUIA_DE_DESPACHO ADD CONSTRAINT FK_RELATIONSHIP_63 FOREIGN KEY (ID_CODIGO_PROVEEDOR)
      REFERENCES CODIGO_PROVEEDOR (ID_CODIGO_PROVEEDOR) ON DELETE RESTRICT ON UPDATE CASCADE;
...
ALTER TABLE DETALLE_ORDEN_DE_COMPRA ADD CONSTRAINT FK_RELATIONSHIP_64 FOREIGN KEY (ID_CODIGO_PROVEEDOR)
      REFERENCES CODIGO_PROVEEDOR (ID_CODIGO_PROVEEDOR) ON DELETE RESTRICT ON UPDATE CASCADE;
...
CREATE VIEW V_RESUMEN_CLIENTE AS 
	SELECT c.ID_CLIENTE, c.NOMBRE_CLIENTE, c.RUT_CLIENTE, c.CORREO_CLIENTE, c.TELEFONO_CLIENTE,
           ...
           coalesce(COUNT(v.ID_VENTA),0) AS VENTAS, coalesce(COUNT(d.ID_DEVOLUCION),0) AS DEVOLUCIONES 	   
 	FROM CLIENTE c 
    LEFT JOIN VENTA v ON c.ID_CLIENTE=v.ID_CLIENTE
    LEFT JOIN DEVOLUCION d ON v.ID_VENTA=d.ID_VENTA
    GROUP BY c.ID_CLIENTE;

DROP VIEW IF EXISTS V_RESUMEN_VENDEDOR;
CREATE VIEW V_RESUMEN_VENDEDOR AS
	...
	FROM USUARIO u
	LEFT JOIN VENTA v ON u.ID_USUARIO=v.ID_VENDEDOR
	LEFT JOIN DEVOLUCION d ON u.ID_USUARIO=d.ID_VENDEDOR
	WHERE u.ROL_USUARIO='V'
    GROUP BY u.ID_USUARIO;

DROP VIEW IF EXISTS V_RESUMEN_BODEGUERO;
CREATE VIEW V_RESUMEN_BODEGUERO AS
	SELECT u.ID_USUARIO, u.NOMBRE_USUARIO, u.RUT_USUARIO,u.email as CORREO_USUARIO, u.TELEFONO_USUARIO,
			(YEAR(SYSDATE())-YEAR(u.FECHA_NACIMIENTO_USUARIO)) AS EDAD,
            COUNT(v.ID_VENTA) AS VENTAS, COUNT(g.ID_GUIA_DESPACHO) AS COMPRAS
    FROM USUARIO u
    LEFT JOIN VENTA v ON u.ID_USUARIO=v.ID_BODEGUERO_DESPACHO
    LEFT JOIN  GUIA_LOCAL_BODEGUERO g ON u.ID_USUARIO=g.ID_BODEGUERO
    WHERE u.ROL_USUARIO='B'
    GROUP BY u.ID_USUARIO;

drop view if exists V_STOCK_INSUMO;
CREATE VIEW V_STOCK_INSUMO AS
	SELECT  i.ID_INSUMO,l.ID_LOCAL,l.DIRECCION_LOCAL,il.STOCK_INSUMO_LOCAL, i.PRECIO_INSUMO, i.NOMBRE_UNICO_INSUMO, ti.NOMBRE_INSUMO
    FROM INSUMO i 
    LEFT JOIN INSUMO_LOCAL il ON  il.ID_INSUMO=i.ID_INSUMO
    LEFT JOIN TIPO_INSUMO ti ON ti.ID_TIPO_INSUMO=i.ID_TIPO_INSUMO
    LEFT JOIN LOCAL l ON l.ID_LOCAL=il.ID_LOCAL
    ORDER BY i.ID_INSUMO;

drop view if exists V_RESUMEN_LOCAL;
CREATE VIEW V_RESUMEN_LOCAL AS
	SELECT l.*,COUNT(ve.ID_USUARIO) AS CANTIDAD_VENDEDORES,COUNT(b.ID_USUARIO) AS CANTIDAD_BODEGUEROS,COUNT(v.ID_VENTA) AS VENTAS_ULTIMO_MES, COUNT(d.ID_DEVOLUCION) AS DEVOLUCIONES_ULTIMO_MES
    FROM LOCAL l
    LEFT JOIN VENTA v ON l.ID_LOCAL=v.ID_LOCAL
    LEFT JOIN DEVOLUCION d ON l.ID_LOCAL=d.ID_LOCAL
    LEFT JOIN USUARIO ve ON l.ID_LOCAL=ve.ID_LOCAL
    LEFT JOIN USUARIO b ON l.ID_LOCAL=b.ID_LOCAL
    WHERE b.ROL_USUARIO = 'B' and ve.ROL_USUARIO = 'V'
    GROUP BY l.ID_LOCAL;
...
drop view if exists V_DEVOLUCION;
CREATE VIEW V_DEVOLUCION AS
    ...
    FROM LOCAL, DEVOLUCION d
    LEFT JOIN VENTA v ON d.ID_VENTA=v.ID_VENTA
    LEFT JOIN CLIENTE c ON v.ID_CLIENTE=c.ID_CLIENTE
    LEFT JOIN USUARIO u ON u.ID_USUARIO=d.ID_VENDEDOR
    LEFT JOIN ESTADO_DEVOLUCION e ON d.ID_ESTADO_DEVOLUCION=e.ID_ESTADO_DEVOLUCION
...
CREATE VIEW V_PROVEEDOR AS
	SELECT p.* , COALESCE(COUNT(c.ID_COTIZACION),0) AS CANTIDAD_COTIZACIONES, COALESCE(COUNT(o.ID_COMPRA),0) AS CANTIDAD_ORDENES_DE_COMPRA
    FROM PROVEEDOR p
    LEFT JOIN ORDEN_DE_COMPRA o ON p.ID_PROVEEDOR=o.ID_PROVEEDOR
    LEFT JOIN COTIZACION c ON p.ID_PROVEEDOR=c.ID_PROVEEDOR
    GROUP BY p.ID_PROVEEDOR;

CREATE VIEW V_COTIZACION AS
	SELECT c.*, p.NOMBRE_PROVEEDOR
    FROM COTIZACION c, PROVEEDOR p
    WHERE c.ID_PROVEEDOR = p.ID_PROVEEDOR; 

CREATE VIEW V_DETALLE_COTIZACION AS
	SELECT d.*,i.NOMBRE_UNICO_INSUMO, c.ID_INSUMO, c.CODIGO_INSUMO
    FROM DETALLE_COTIZACION d, INSUMO i, CODIGO_PROVEEDOR c
    WHERE d.ID_CODIGO_PROVEEDOR=c.ID_CODIGO_PROVEEDOR and c.ID_INSUMO=i.ID_INSUMO;
...
CREATE VIEW V_DETALLE_COMPRA AS
	SELECT d.*,i.NOMBRE_UNICO_INSUMO, c.ID_INSUMO, c.CODIGO_INSUMO
    FROM DETALLE_ORDEN_DE_COMPRA d, INSUMO i, CODIGO_PROVEEDOR c
    WHERE d.ID_CODIGO_PROVEEDOR=c.ID_CODIGO_PROVEEDOR and c.ID_INSUMO=i.ID_INSUMO;
...
CREATE VIEW V_DETALLE_GUIA AS
	SELECT d.*, i.NOMBRE_UNICO_INSUMO,  c.ID_INSUMO, c.CODIGO_INSUMO
    FROM DETALLE_GUIA_DE_DESPACHO d,INSUMO i, CODIGO_PROVEEDOR c
    WHERE d.ID_CODIGO_PROVEEDOR=c.ID_CODIGO_PROVEEDOR and c.ID_INSUMO=i.ID_INSUMO;
...
DROP VIEW IF EXISTS V_CODIGO_PROVEEDOR;
CREATE VIEW V_CODIGO_PROVEEDOR AS
	SELECT c.*,p.NOMBRE_PROVEEDOR,i.NOMBRE_UNICO_INSUMO
    FROM CODIGO_PROVEEDOR c, PROVEEDOR p, INSUMO i
...
BEGIN
    ...
    DECLARE actualizar_stock CURSOR FOR
    SELECT c.id_insumo,d.cantidad_guia 
	...
	#revisa si se completo la orden de compra al cambiar su estado
