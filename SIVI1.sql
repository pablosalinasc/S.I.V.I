...
CREATE VIEW V_RESUMEN_CLIENTE AS 
	SELECT c.ID_CLIENTE, c.NOMBRE_CLIENTE, c.RUT_CLIENTE, c.CORREO_CLIENTE, c.TELEFONO_CLIENTE,
           DATE(SYSDATE()) -c.FECHA_NACIMIENTO_CLIENTE AS EDAD, c.DESCUENTO_CLIENTE,
...
CREATE VIEW V_RESUMEN_VENDEDOR AS
	SELECT u.ID_USUARIO,u.NOMBRE_USUARIO, u.RUT_USUARIO, u.CORREO_USUARIO, u.TELEFONO_USUARIO,
			DATE(SYSDATE())-u.FECHA_NACIMIENTO_USUARIO AS EDAD,
			COUNT(v.ID_VENTA) AS VENTAS, COUNT(d.ID_DEVOLUCION) AS DEVOLUCIONES 
...
CREATE VIEW V_RESUMEN_BODEGUERO AS
    ...
    WHERE u.ID_USUARIO=g.ID_BODEGUERO AND u.ID_USUARIO=v.ID_BODEGUERO_DESPACHO AND u.ROL_USUARIO='B'
    GROUP BY u.ID_USUARIO;

CREATE VIEW V_STOCK_INSUMO AS
	SELECT  il.ID_INSUMO,l.DIRECCION_LOCAL,il.STOCK_INSUMO_LOCAL
    FROM LOCAL l, INSUMO_LOCAL il
    WHERE l.ID_LOCAL=il.ID_LOCAL
...
CREATE VIEW V_VENTA AS
	SELECT v.ID_VENTA, u.NOMBRE_USUARIO AS NOMBRE_VENDEDOR, c.NOMBRE_CLIENTE AS NOMBRE_CLIENTE,l.DIRECCION_LOCAL
